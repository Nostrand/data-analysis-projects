# Data Analysis Projects

## Description

This repository serves as a collection of various small-scale data analysis projects. Each project focuses on distinct datasets and explores various aspects of data analysis using different tools and techniques, including the R programming language, Python, SQL and Power BI.

## Projects

### Project 1: Investigating Netflix Movies

<img src="investigating-netflix-movies/images/plot.png" alt="Investigating Netflix Movies" width="800">

**Description:** This project involves exploratory analysis of the duration of Netflix movies. The goal is to perform a basic analysis to gain insights into the trends in movie durations.

**Skills & Concepts:** Data Transformation, Data Visualization

**Tools:** Python (Pandas, Matplotlib), Jupyter Notebook

**Notebook:** [Investigating Netflix Movies Notebook](https://github.com/Nostrand/data-analysis-projects/blob/main/investigating-netflix-movies/notebook.ipynb)

---
### Project 2: Analyzing NASA Planetary Exploration Budgets

<img src="analyzing-nasa-planetary-exploration-budgets/plot2.png" alt="NASA Budget Analysis" width="800">

**Description:** This project focuses on the analysis of NASA budgets data related to planetary exploration. It utilizes SQL queries to extract, transform, and analyze the budget data, providing insights into funding trends and allocation across different exploration programs.

**Skills & Concepts:** Exploratory Data Analysis

**Tools:** Python (Pandas, Matplotlib), SQL, Jupyter Notebook

**Notebook:** [Analyzing NASA Planetary Exploration Budgets Notebook](https://github.com/Nostrand/data-analysis-projects/blob/main/analyzing-nasa-planetary-exploration-budgets/notebook.ipynb)

---
### Project 3: Case Study HR Analytics

<img src="case-study-hr-analytics-in-power-bi/images/overview.png" alt="HR Analytics Dashboard" width="800">

**Description:** This case study delves into HR analytics and dashboard design, exploring a fictitious HR dataset. The project aims to gain insights into employee metrics and provide interactive dashboards for HR decision-making.

**Skills & Concepts:** Data Transformation, DAX measures, Data Visualization, Dashboard Design

**Tools:** Power BI

**Report:** [Case Study HR Analytics Report](https://github.com/Nostrand/data-analysis-projects/blob/main/case-study-hr-analytics-in-power-bi/case-study-hr-analytics.md)

---
### Project 4: CO2 Emissions and Bicycle Market Analysis

<img src="co2-emissions-and-bicycle-market-analysis/images/average-co2-emission.png" alt="Average CO2 Emissions catplot" width="800">

**Description:**  This project explores CO2 emission data for Canadian vehicles and analyze a database from a chain of bicycle stores using Python and SQL. The goal is to gain insights into vehicles emission trends and bicycle market preferences.

**Skills & Concepts:** Exploratory Data Analysis

**Tools:** Python (Pandas, Seaborn), SQL, Jupyter Notebook

**Notebook:** [CO2 Emissions and Bicycle Market Analysis](https://github.com/Nostrand/data-analysis-projects/blob/main/co2-emissions-and-bicycle-market-analysis/co2-emissions-and-bicycle-market-analysis.ipynb)

---
### Project 5: Analyzing Customer Churn (Power BI)

<img src="case-study-analyzing-customer-churn/Images/overview.png" alt="customer churn dashboard overview page" width="800">

**Description:**  This case study dives into customer churn analytics, using a simulated dataset from a fictitious Telecom provider called Databel. The goal is to uncover the reasons behind customer departures and identify factors influencing churn, to empower stakeholders to make informed decisions and improve customer retention.

**Skills & Concepts:** Data Transformation, DAX measures, Data Visualization, Dashboard Design

**Tools:** Power BI

**Report:** [Analyzing Customer Churn](https://github.com/Nostrand/data-analysis-projects/blob/main/case-study-analyzing-customer-churn/case-study-analyzing-customer-churn.md)
